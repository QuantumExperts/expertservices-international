...
exports.handler = async (event) => {
  ...
  try {
    ...
    const nominatingName = (body.nominatingName || 'Unknown').trim();
    const organisation = (body.organisation || 'Individual').trim();
    ...
    const folderBase = `${nominatingName} - ${organisation}`.replace(/[<>:"/\\|?*]/g, '_').trim();
    const folderName = `${dateStr} ${folderBase}`;

    // Ensure nomination folder exists
    ...
    console.error('Create nomination sessions error:', err.message);
    return {
      statusCode: 500,
      headers,
